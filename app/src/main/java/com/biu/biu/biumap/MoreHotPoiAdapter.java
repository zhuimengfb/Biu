...
public class MoreHotPoiAdapter extends BaseAdapter {
	 ...
	 * 
	 * @param context
	 *            :运行上下文，这里是MoreHotActivity
	 * @param listItems
	 *            ：帖子信息存储
	 */
	public MoreHotPoiAdapter(Context context, ArrayList<TipItemInfo> listItems,
			Handler handler, double lat, double lng) {
	...
	public class DownbtnListener implements OnClickListener {
		...
		public void onClick(View v) {
			...
			// 更新显示ListView的状态
			MoreHotPoiAdapter.this.notifyDataSetChanged();
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (mlistItemsinfo.get(position).isEmpty) {
			// 无数据
			listItemView.homeContenttv.setText("暂无更多热帖");
			...
			// imageGetter, null));
			// }else{
			// listItemView.homeContenttv.setText(mlistItemsinfo.get(position).get("itemContent").toString());
			// }
